...
class CisEvidence(models.Model):
    evidence_id = models.AutoField(primary_key=True)
    course_id = models.ForeignKey(Course, on_delete=models.PROTECT, db_column='course_id')
    academic_year = models.CharField(max_length=9)
    semester = models.IntegerField()
    assessment_tool = models.CharField(max_length=50) # e.g., FA-PR, SLA
    ...
    def __str__(self):
        return f"{self.assessment_tool} - {self.course_id.course_code} ({self.academic_year})"
