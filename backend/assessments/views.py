...
class MarksListCreateAPIView(APIView):
    def get(self, request) -> Response:
        ...
        marks = MarksEntry.objects.filter(
            assessment_id__course_id=course_id,
            assessment_id__is_active=True
        )
    ...
    def post(self, request) -> Response:
        ...
        with transaction.atomic():
            for item in marks_data:
                ...
                if student_id is not None and marks_obtained is not None:
                    MarksEntry.objects.update_or_create(
                        assessment_id=assessment,
                        student_id_id=student_id,
                        defaults={'marks_obtained': marks_obtained, 'entered_by': set_by}
                    )
