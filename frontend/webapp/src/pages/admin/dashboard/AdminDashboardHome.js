            ...
            <div className="container-fluid bg-white rounded shadow-sm p-4 dashboard-container">
                ...
                {(attention_required.config.length > 0 || attention_required.structural.length > 0) && (
                    <div className="row mb-4 px-2">
                        <div className="col-12 attention-required-box p-3 rounded">
                            <h6 className="text-danger fw-bold mb-3 d-flex align-items-center gap-2">
                                <FaRegClock /> Attention Required
                            </h6>
                            <div className="row">
                                {attention_required.config.length > 0 && (
                                    <div className={`col-md-6 ${attention_required.structural.length > 0 ? 'border-end' : ''}`}>
                                        <span className="small text-muted text-uppercase mb-2 d-block">Configuration Issues</span>
                                        <ul className="list-unstyled mb-0">
                                            {attention_required.config.map((issue, i) => (
                                                <li key={i} className="small text-danger mb-1">• {issue}</li>
                                            ))}
                                        </ul>
                                    </div>
                                )}
                                {attention_required.structural.length > 0 && (
                                    <div className={`col-md-6 ${attention_required.config.length > 0 ? 'px-4' : ''}`}>
                                        <span className="small text-muted text-uppercase mb-2 d-block">Structural Issues</span>
                                        <ul className="list-unstyled mb-0">
                                            {attention_required.structural.map((issue, i) => (
                                                <li key={i} className="small text-danger mb-1">• {issue}</li>
                                            ))}
                                        </ul>
                                    </div>
